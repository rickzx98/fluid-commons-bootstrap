...
export const BookAdditionalInformation = ({ settings, managedBook }) => {
    return (<span>
        ...
        <TextInput label={LABEL_INTERNET_RESOURCE} name={Book.INTERNET_RESOURCE} value={managedBook[Book.INTERNET_RESOURCE]} />
        <TextArea label={LABEL_GENERAL_NOTE} name={Book.GENERAL_NOTE} rows={4} value={managedBook[Book.GENERAL_NOTE]} />
        <Selector options={getReadingLevelForDropdown()} label={LABEL_READING_LEVEL} name={Book.READING_LEVEL} value={managedBook[Book.GENERAL_NOREADING_LEVELTE]} />
        <Selector options={resourceTypesForDropdown(settings)} label={LABEL_RESOURCE_TYPE} name={Book.RESOURCE_TYPE} value={managedBook[Book.RESOURCE_TYPEADING_LEVEL]} />
    </span>);
};
